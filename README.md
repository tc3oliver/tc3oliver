...
| | |
|---|---|
| [**LLM Inference Systems Research**](https://github.com/tc3oliver/llm-inference-systems) | One completed study and four research threads, on one Apple silicon machine. Built a heterogeneous ANE prefill path, SpecPrefill (attention-based sparse prefill) on top of it, incremental dense-prefix recovery and a foreground-priority scheduler. Cold 16K time-to-first-token fell from 57.84 s to 19.24 s — and a request-level trace then showed a prefix-cache cliff it never repaired, costing more over a session than it saved. An earlier bake-off had shown the same signature: 7.2× the tokens recomputed, 63.1% cache hit against 99.5%. Across 431 drafted sequences, speculative-decoding acceptance tracks the model, not the task. Also found a protected-prefix correctness bug. [Article →](https://github.com/tc3oliver/llm-inference-systems/blob/main/publications/article.zh.md) · [Engineering →](https://github.com/tc3oliver/llm-inference-systems/blob/main/ENGINEERING.md) · [Case study →](https://meowcoder.com/work/reusable-state-economics/) · [omlx#3756 →](https://github.com/jundot/omlx/pull/3756) · [omlx#3762 →](https://github.com/jundot/omlx/pull/3762) (both open) |
